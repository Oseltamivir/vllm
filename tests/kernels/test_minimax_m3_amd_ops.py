...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"ep_size": 8}, True),
        ({"ep_size": 8, "max_model_len": 9472}, False),
        ({"ep_size": 4}, False),
...
@pytest.mark.parametrize(
    ("num_valid_tokens", "num_local_experts", "block_m", "allocation", "expected"),
    [
        (864, 16, 16, 2784, 1104),
        (64, 8, 16, 1024, 176),
        (4, 128, 16, 64, 64),
...
def test_mxfp8_max_post_padded(
    num_valid_tokens,
    num_local_experts,
    block_m,
    allocation,
...
@requires_mi3xx
@torch.inference_mode()
def test_moe_fused_sum_skips_remote_weighted_routes():
    ...
    weights = torch.rand(T, top_k, device=DEVICE, dtype=torch.float32)
    routes = torch.randn(T, top_k, H, device=DEVICE, dtype=torch.bfloat16)
    weighted_routes = (routes.float() * weights[..., None]).to(torch.bfloat16)
    weighted_routes[:, 1::2] = torch.nan
    ...
    ref = weighted_routes[:, ::2].float().sum(dim=1).to(torch.bfloat16)
...
@pytest.mark.skipif(
    not current_platform.is_fp8_fnuz(),
    reason="route-aware native EP optimization is specific to gfx94x.",
)
@torch.inference_mode()
def test_mxfp8_native_moe_expert_parallel():
    from vllm.model_executor.layers.fused_moe.experts.mxfp8_native_moe import (
        fused_moe_mxfp8_native,
    )

    torch.manual_seed(0)
    T, H, inter, global_e, local_e, top_k = 8, 256, 512, 8, 4, 2
    alpha, beta, limit = 1.702, 1.0, 7.0
    w13_bf16 = (
    ...
    w13_fp8, w13_scale = normalize_mxfp8_e4m3fn_to_e4m3fnuz(
        w13_fp8,
        w13_scale,
    )
    w2_fp8, w2_scale = normalize_mxfp8_e4m3fn_to_e4m3fnuz(
        w2_fp8,
        w2_scale,
    )
    w13_deq = dequant_mxfp8_to_bf16(w13_fp8, w13_scale)
