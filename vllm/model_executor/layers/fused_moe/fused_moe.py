...
def fused_moe_gated_kernel(
    ...
    """BF16 grouped GEMM1 with a split SwiGLU-OAI epilogue."""
    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(EM, BLOCK_SIZE_M)
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N)
    num_pid_in_group = GROUP_SIZE_M * num_pid_n
    ...
    b_up_ptrs = b_gate_ptrs + N * stride_bn
    ...
    # Preserve the BF16 GEMM1 store/reload boundary of the unfused path before
    # applying SwiGLU in FP32.
...
def invoke_fused_moe_gated_triton_kernel(
    A: torch.Tensor,
    B: torch.Tensor,
    C: torch.Tensor,
    ...
    """Run BF16 GEMM1 and emit activated split-SwiGLU rows directly."""
    assert A.dtype == B.dtype == C.dtype == torch.bfloat16
    assert B.size(1) == C.size(1) * 2
    ...
    fused_moe_gated_kernel[grid](
        ...
        alpha,
        beta,
        0.0 if limit is None else limit,
        top_k=top_k,
