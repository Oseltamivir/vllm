...
def _index_block_score_kernel(
    ...
    if USE_FP8_DOT:
        ...
        q_absmax = tl.maximum(tl.max(tl.abs(q_fp32), axis=1), 1e-4)
        fp8_max: tl.constexpr = 224.0 if IS_FNUZ else 448.0
        q_scale = tl.exp2(tl.ceil(tl.log2(q_absmax / fp8_max)))
        q = (q_fp32 / q_scale[:, None]).to(ik_cache_ptr.dtype.element_ty)
    ...
    for i in tl.range(0, hi, BLOCK_SIZE_K):
        ...
        if USE_FP8:
            ...
            k = k_fp8 if USE_FP8_DOT else k_fp8.to(tl.bfloat16)
        else:
            k = tl.load(
                ik_cache_ptr
                + page * stride_ik_blk
...
def _decode_index_score_kernel(
    ...
    if USE_FP8_DOT:
        q_fp32 = q.to(tl.float32)
        q_absmax = tl.maximum(tl.max(tl.abs(q_fp32), axis=0), 1e-4)
        fp8_max: tl.constexpr = 224.0 if IS_FNUZ else 448.0
        q_scale = tl.exp2(tl.ceil(tl.log2(q_absmax / fp8_max)))
        q = (q_fp32 / q_scale[None, :]).to(ik_cache_ptr.dtype.element_ty)
...
def minimax_m3_index_score(
    ...
    use_fp8_dot = use_fp8 and current_platform.is_rocm()
    if use_fp8:
        assert index_kv_cache.shape[-1] == head_dim + 4
        # Explicit FNUZ casts are unsupported by ROCm Triton. Loading through
        # a platform-FP8 pointer gives Triton the correct element type while
        # preserving the byte-packed scale trailer in each cache page.
        kernel_index_cache = index_kv_cache.view(current_platform.fp8_dtype())
    else:
